...
def error_character(input_char, char_range):
  ...
  if (char_range == 'digit'):
    ...
    #
    elif (input_char.isalpha()) and       (rand_num <= (single_typo_prob['same_row'] +                     single_typo_prob['same_col'])):
    ...
    if (rand_num <= single_typo_prob['same_row']):
      if (input_char in rows):
        ...
        choice_str =  string.replace(string.lowercase+string.digits,                                     input_char, '')
...
def first_day_of_year(year):
  ...
  if (year == 0):
    ...
    if (year > 1600):
      leap_adj = leap_adj - int((year + 99 - 1600) / 100) +                 int((year + 399 - 1600) / 400)
...
def epoch_to_date(daynum):
  ...
  if (not (isinstance(daynum, int) or isinstance(daynum, long))):
    # print 'Error: Input value for "daynum" is not of integer type: %s' %          (str(daynum))
    raise Exception
...
def date_to_epoch(day, month, year):
  ...
  #
  if (year_int <= 1000):
    # print 'Error: Input value for "year" is not a positive integer ' +          'number: %i' % (year)
    raise Exception

  leap_year_flag = is_leap_year(year_int)

  if (month_int <= 0) or (month_int > 12):
    # print 'Error: Input value for "month" is not a possible day number: %i' %          (month)
    raise Exception
...
def load_misspellings_dict(misspellings_file_name):
  ...
  try:
    ...
    # print 'Error: Can not read from misspellings file "%s"' %          (misspellings_file_name)
    raise IOError
  ...
  for line in file_data:
    ...
    if (len(l) > 0) and (l[0] != '#'):  # Not empty line and not comment
      ...
      if (len(ll) == 2):  # Line contains a key - - - - - - - - - - - - - - - -
        ...
        if (vals == ''):
          # print 'Error: No misspellings given for "%s" in line: "%s"' %                (key, l)
          raise Exception
        ...
        #
        if (key in val_set):
          # print 'Error: A misspelling is the same as the original value' +                ' "%s" in line: "%s"' % (key, l)
          raise Exception
  ...
  for line in file_data:
    ...
    if (len(l) > 0) and (l[0] != '#'):  # Not empty line and not comment
      ...
      if (len(ll) == 2):  # Line contains a key - - - - - - - - - - - - - - - -
        ...
        if (vals == ''):
          # print 'Error: No lookupings given for "%s" in line: "%s"' %                (key, l)
          raise Exception
...
def get_transformation(s, t):
  ...
  def slavo_germanic(str):
    if ((str.find('w') > -1) or (str.find('k') > -1) or        (str.find('cz') > -1) or (str.find('witz') > -1)):
      return 1
    else:
      return 0
  ...
  def do_collect_replacement(s, where, orgpat, newpat, precond, postcond,
    ...
    while ((orgpat in tmpstr[start_search:]) and (stop == False)):
      ...
      if (precond == 'None'):
        ...
        if (((precond == 'V') and (tmpstr[pat_start-1] in vowels)) or            ((precond == 'C') and (tmpstr[pat_start-1] not in vowels))):
          OKpre = True
      ...
      if (postcond == 'None'):
        ...
        if (pat_end < str_len):
          if (((postcond == 'V') and (tmpstr[pat_end] in vowels)) or              ((postcond == 'C') and (tmpstr[pat_end] not in vowels))):
            OKpost = True
      ...
      #
      if ((OKpre == True) and (OKpost == True) and (OKexist == True) and          (OKstart == True)) and (((where == 'START') and (pat_start == 0))          or ((where == 'MIDDLE') and (pat_start > 0) and          (pat_start+pat_len < str_len)) or ((where == 'END') and          (pat_start+pat_len == str_len)) or (where == 'ALL')):
...
def apply_change(str_to_change, ch):
  ...
  if (org_pat_start == 0):
    ...
    workstr = workstr[0:org_pat_start] + subs +              workstr[org_pat_start+org_pat_length:]
...
for field_dict in field_list:
  ...
  if (field_dict['type'] == 'freq'):
    ...
    if (not (field_dict.has_key('start_date') and             field_dict.has_key('end_date'))):
    ...
    if (not (field_dict.has_key('area_codes') and             field_dict.has_key('num_digits'))):
      ...
      if (not isinstance(field_dict['area_codes'],list)):
        # print 'Error: Area codes given are not a string or a list: %s' %              (str(field_dict['area_codes']))
        raise Exception

      if (not isinstance(field_dict['num_digits'],int)):
        # print 'Error: Number of digits given is not an integer: %s (%s)' %              (str(field_dict['num_digits']), type(field_dict['num_digits']))
        raise Exception

      field_list[i] = field_dict

  elif (field_dict['type'] == 'ident'):
    if (not (field_dict.has_key('start_id') and             field_dict.has_key('end_id'))):
      # print 'Error: Field of type "iden" has no start and/or end ' +            'identification number given'
      raise Exception
  ...
  if (field_prob_sum > 0.0) and (abs(field_prob_sum - 1.0) > 0.001):
      # print 'Error: Sum of probabilities for field "%s" is not 1.0: %f' %            (field_dict['name'], field_prob_sum)
      raise Exception
...
# print
# print 'Create %i original and %i duplicate records' %      (num_org_records, num_dup_records)
# print '  Distribution of number of duplicates (maximal %i duplicates):' %      (max_num_dups)
...
for field_dict in field_list:
  ...
  if (field_dict['type'] == 'freq'):  # Check for 'freq' field type
    ...
    if (file_name != None):
      ...
      if (VERBOSE_OUTPUT == True):
        # print '  Loaded frequency file for field "%s" from file: %s' %              (field_dict['name'], file_name)
        # print
        pass

    else:
      # print '  Error: No file name defined for frequency field "%s"' %            (field_dict['name'])
      raise Exception
  ...
  if ('misspell_file' in field_dict):  # Load misspellings dictionary file
    ...
    if (VERBOSE_OUTPUT == True):
      # print '  Loaded misspellings dictionary for field "%s" from file: "%s'            % (field_dict['name'], misspell_file_name)
      # print
      pass
  ...
  if ('lookup_file' in field_dict):  # Load lookup dictionary file
    ...
    if (VERBOSE_OUTPUT == True):
      # print '  Loaded lookup dictionary for field "%s" from file: "%s'            % (field_dict['name'], lookup_file_name)
      # print
      pass
...
while (rec_cnt < num_org_records):
  ...
  for field_dict in field_list:
    ...
    if (random.random() <= field_dict['miss_prob']):
      ...
      # Check for dependencies and follow if a certain probability is given
      #
      if ((field_dict['name'] == 'date_of_birth') and          (random.random() < field_dict['depend_prob']) and          (rec_dict.get('age', None) != None)):
  ...
  if (rec_str not in all_rec_set):  # Check if same record already created
    ...
    if (VERBOSE_OUTPUT == True):
      ...
      for field_name in field_names:
        pass
        # print '    %-18s: %-30s' % (field_name,                                    rec_dict.get(field_name, missing_value))
      # print
...
if (num_of_hofam_duplicate > 0):
  ...
  while (rec_cnt < num_of_hofam_duplicate):
    ...
    while ((org_rec_id in org_hofam_rec_used) or           (org_rec_id not in org_rec) or (rec_age < min_age) or           (all_fields_flag == False)):
...
## PC 15/12 OK above #######################
      ...
      if (num_dups > 0):
        ...
        if (family_role=='husband'):
          ...
          if (random.random() <= prob_create_role['wife']):
            ...
            if (random.random() <= prob_live_family['husband_with_wife' +                                                    str_key]):
              family_role_to_create_live.insert(                                           len(family_role_to_create_live),'y')
            else:
              family_role_to_create_live.insert(                                           len(family_role_to_create_live),'n')

          if ((random.random() <= prob_has_children['parent_has_children' +              str_key]) and (max_num_of_children > 0)):
            ...
            while (numrole < max_num_of_children):
              ...
              if (random.random() <=  prob_create_role[str_children]):
                family_role_to_create.insert(len(family_role_to_create),                                             str_children)
                numrole+=1

                if (random.random()<=prob_live_family['parent_with_children'                                                      + str_key]):
                  family_role_to_create_live.insert(                                           len(family_role_to_create_live),'y')
                else:
                  family_role_to_create_live.insert(                                           len(family_role_to_create_live),'n')
          ...
          if (random.random() <= prob_create_role['father']):
            ...
            if (random.random() <= prob_live_family['son_with_father' +                                                    str_key]):
              family_role_to_create_live.insert(                                           len(family_role_to_create_live),'y')
            else:
              family_role_to_create_live.insert(                                           len(family_role_to_create_live),'n')
          ...
          if (random.random() <= prob_create_role['mother']):
            ...
            if (random.random() <= prob_live_family['son_with_mother' +                                                    str_key]):
              family_role_to_create_live.insert(                                           len(family_role_to_create_live),'y')
            else:
              family_role_to_create_live.insert(                                           len(family_role_to_create_live),'n')
          ...
          while (numrole < max_num_of_children):
            ...
            if (random.random() <=  prob_create_role[str_children]):
              ...
              if (random.random() <= prob_live_family['brother_with_' +                                                      str_children+str_key]):
                family_role_to_create_live.insert(                                           len(family_role_to_create_live),'y')
              else:
                family_role_to_create_live.insert(                                           len(family_role_to_create_live),'n')
          ...
          if (random.random() <= prob_create_role['father']):
            ...
            if (random.random() <= prob_live_family['daughter_with_father' +                                                    str_key]):
              family_role_to_create_live.insert(                                           len(family_role_to_create_live),'y')
            else:
              family_role_to_create_live.insert(                                           len(family_role_to_create_live),'n')
          ...
          if (random.random() <= prob_create_role['mother']):
            ...
            if (random.random() <= prob_live_family['daughter_with_mother' +                                                    str_key]):
              family_role_to_create_live.insert(                                           len(family_role_to_create_live),'y')
            else:
              family_role_to_create_live.insert(                                           len(family_role_to_create_live),'n')
          ...
          while (numrole < max_num_of_children):
            ...
            if (random.random() <= prob_create_role[str_children]):
              ...
              if (random.random() <= prob_live_family[str_children +                                                      '_with_sister'+str_key]):
                family_role_to_create_live.insert(                                           len(family_role_to_create_live),'y')
              else:
                family_role_to_create_live.insert(                                           len(family_role_to_create_live),'n')
    ...
    while (dup_count < num_dups) and (rec_cnt < num_of_hofam_duplicate):
      ...
      if (family_flag == True):
        ...
        if ((field_name in parent_address) and            ('father' not in family_role_to_create) and            ('mother' not in family_role_to_create)):
      ...
      for field_dict in field_list:
        ...
        if (family_flag == True):
          ...
          if ('age' in dup_rec_dict):
            ...
            if (family_role_to_create[dup_count] in                ['son','daughter','brother','sister']):
              #brother sister
              if ((family_role_to_create[dup_count] in ['brother']) and                  (family_role in ['son'])):
                str_key1='brother_with_'
                str_key2='brother'
                str_key3='son'
              elif ((family_role_to_create[dup_count] in ['brother']) and                    (family_role in ['daughter'])):
                str_key1='brother_with_'
                str_key2='sister'
                str_key3='son'

              if ((family_role_to_create[dup_count] in ['sister']) and                  (family_role in ['son'])):
                str_key1='sister_with_'
                str_key2='brother'
                str_key3='daughter'
              elif ((family_role_to_create[dup_count] in ['sister']) and                    (family_role in ['daughter'])):
                str_key1='sister_with_'
                str_key2='sister'
                str_key3='daughter'

              if ((family_role_to_create[dup_count] in ['son','daughter'])                  and (family_role in ['husband','wife'])):
              ...
              if (family_role_to_create[dup_count] in ['brother','sister']):
                 if ((age_value >= age_min_move) and (random.random() <=                     prob_live_family[str_key1+str_key2+str_key])):
                 ...
                 if ((age_value >= age_min_move) and (random.random() <=                     prob_live_family[family_role_to_create[dup_count] +                     '_with_'+str_key1+str_key])):
                   family_role_to_create_live[dup_count] = 'n'
                 else:
                   family_role_to_create_live[dup_count] = 'y'
              ...
              if ((family_role_to_create[dup_count] in ['son','daughter'])                  and (family_role_to_create_live[dup_count] == 'y')):
                #son daughter
                list_live_with1 = []
                for i in range(int(prob_live_family[family_role_to_create[                               dup_count]+'_with_father'+str_key]*100)):
                  list_live_with1 += 'father'
                for i in range(int(prob_live_family[family_role_to_create[                               dup_count]+'_with_mother'+str_key]*100)):
                  list_live_with1 += 'mother'
                live_with = random.choice(list_live_with1)
              elif ((family_role_to_create[dup_count] in ['brother','sister'])                   and (family_role in ['son','daughter']) and                    (family_role_to_create_live[dup_count] == 'y')):
                list_live_with2 = []
                for i in range(int(prob_live_family[str_key1+                                                    str_key2+str_key]*100)):
                  list_live_with2 += 'sibling'
                for i in range(int(prob_live_family[str_key3+                                                 '_with_father'+str_key]*100)):
                  list_live_with2 += 'father'
                for i in range(int(prob_live_family[str_key3+                                                 '_with_mother'+str_key]*100)):
          ...
          if (field_dict['type'] == 'freq'):  # A frequency file based field
            ...
            if (family_flag == False) and (field_name not in ['culture']):
              ...
              if (field_name in ['age','sex']):
                str_key='household_'
              rand_num = random.randint(0, freq_files_length[str_key+                                                             field_name]-1)
            ...
            if (family_flag == True):
              if (field_name == 'culture'):
                ...
                if family_role_to_create[dup_count] in                                          ['husband','father','son','brother']:
                  rand_val = 'm'
                elif family_role_to_create[dup_count] in                                         ['wife','mother','daughter','sister']:
                  rand_val = 'f'
              elif (field_name == 'age'):
                if family_role_to_create[dup_count] in ['father','mother']:
                  if family_role_to_create[dup_count] in ['father']:
                    gap = random_select(set_distribution(age_gap[                              'min_father_with_children'],                              age_gap['max_father_with_children'],                              prob_distribution_age_gap))+                              age_gap['min_father_with_children']
                  else:
                    gap = random_select(set_distribution(age_gap[                              'min_mother_with_children'],                              age_gap['max_mother_with_children'],                              prob_distribution_age_gap))+                              age_gap['min_mother_with_children']
                  ...
                  while (int(rand_val) < 17):  # Minimum age
                    ...
                    gap = random_select(set_distribution(                              age_gap['min_husband_with_wife'],                              age_gap['max_husband_with_wife'],                              prob_distribution_age_gap))+                              age_gap['min_husband_with_wife']
                    if (sign_val=='+'):
                      rand_val = str(int(age) + gap)
                    else:
                      rand_val = str(int(age) - gap)
                  ...
                  for fr in family_age_rec:
                     if ((int(age) > family_age_rec[fr]) and                         (fr in ['father','mother','husband','wife'])):
                  ...
                  while (int(rand_val) <= 0):
                    sign_val = random.choice(list_age_gap)
                    gap = random_select(set_distribution(                              age_gap['min_sibling'],                              age_gap['max_sibling'],                              prob_distribution_age_gap))+                              age_gap['min_sibling']
            ...
            if (family_flag == True):
              ...
              if ((family_role_to_create[dup_count] in ['son','daughter'])                  and (family_role_to_create_live[dup_count] == 'y')):
                ...
                if ((father_mother_live_together_flag) and                    (('father' in family_role_to_create) and                    ('mother' in family_role_to_create))):
                  parent_address = parent_father_address
                else:
                  if ((live_with == 'father') and                      ('father' in family_role_to_create)):
                    parent_address = parent_father_address
                  elif ((live_with == 'mother') and                        ('mother' in family_role_to_create)):
                    parent_address = parent_mother_address
                  else:
                    parent_address = parent_sibling_address
              elif ((family_role_to_create[dup_count] in ['mother']) and                    (family_role_to_create_live[dup_count] == 'y')):
                if ((father_mother_live_together_flag) and                    (('father' in family_role_to_create) and                    ('mother' in family_role_to_create))):
                  parent_address = parent_father_address
                else:
              ...
              elif ((family_role_to_create[dup_count] in ['wife']) and                    (family_role_to_create_live[dup_count] == 'y')):
                if ((father_mother_live_together_flag) and                    (('husband' in family_role_to_create) and                    ('wife' in family_role_to_create))):
                  parent_address = parent_father_address
                else:
              ...
              do_depend=True
              if ((field_name in parent_address) and                  (family_role_to_create[dup_count] in ['brother','sister'])                  and (family_role_to_create_live[dup_count] in ['y'])):
                rand_val = parent_address[field_name]
                do_depend = False
              elif ((field_name in parent_address) and                    (family_role_to_create[dup_count] in ['mother']) and                    (family_role_to_create_live[dup_count] in ['n']) and                    ('father' in family_role_to_create) and                    father_mother_live_together_flag):
            ...
            if (family_flag == False):
              if ((field_dict['name']=='date_of_birth') and                  (random.random() < field_dict['depend_prob'])):
                year_birth = current_year -                               int(dup_rec_dict['age'].replace(blank_value,''))
                rand_val   = str(year_birth)+rand_date[1]+rand_date[0]
                if ((random.random() > age_dict['depend_prob']) and                    (dup_rec_dict['age'].find(blank_value) < 0)):
                  rand_num = random.randint(0, freq_files_length['age']-1)
            ...
            if (family_flag == True):
              ...
              if ((family_role_to_create[dup_count] in ['son','daughter'])                  and (family_role_to_create_live[dup_count] == 'y')):
                if ((father_mother_live_together_flag) and                    (('husband' in family_role_to_create) and
            ...
            if (family_flag == True):
              if (field_name == 'family_role'):
                rand_val = family_role_to_create[dup_count] + '-' +                           family_role_to_create_live[dup_count] + '-' +                           str(father_mother_live_together_flag)

        dup_field_val = rand_val

        # Now check if the modified field value is different - - - - - - - - -
        #
        if (old_field_val == org_field_val) and           (dup_field_val != old_field_val):  # The first field modification
           field_mod_count_dict[field_name] = 1
           num_modif_in_record += 1
        elif ((old_field_val != org_field_val) and              (dup_field_val != old_field_val)):  # Following modifications
           field_mod_count_dict[field_name] += 1
           num_modif_in_record += 1
        if ((dup_field_val != old_field_val) and            (field_name not in field_filter)) or (field_name in field_filter):
        ...
        if (family_flag == True):
          if (field_name in parent_address):
            ...
            elif (((family_role_to_create[dup_count] in ['mother']) and                  ('father' not in family_role_to_create)) or                  ((family_role_to_create[dup_count] in ['wife']) and                  ('husband' not in family_role_to_create))):
      ...
      if (rec_str not in all_rec_set):  # Check if same record already created
        ...
        if (family_flag == True):
          ...
          if ((family_role in ['wife']) and              (org_rec[org_rec_id]['title'].lower() in ['miss', 'ms'])):
        ...
        if (VERBOSE_OUTPUT == True):
         ...
         # print '    Number of modifications in record: %d' %               (num_modif_in_record)
         # print '    Record ID         : %-30s | %-30s' %               (org_rec_dict['rec_id'], dup_rec_dict['rec_id'])
          for field_name in field_names:
            pass
           # print '    %-18s: %-30s | %-30s' %                 (field_name, org_rec_dict.get(field_name, missing_value),                  dup_rec_dict.get(field_name, missing_value))
         # print
  ...
  while (rec_cnt < num_dup_records):
    ...
    if (type_modification=='all'):  # Randomly select an error type according
      ...
      for error_type in error_type_distribution:
        list_type_of_error += [error_type] *                               int(error_type_distribution[error_type]*100)
      ...
      if (max_num_record_modifi > 1):
        ...
        if (type_modification_to_apply == 'typ'):
          ...
          for field_pair in field_swap_pair_list:
            ...
            if (random.random() <= field_swap_prob[field_pair]) and               (num_modif_in_record <= (max_num_record_modifi-2)):
              ...
              if (fname_a in dup_rec_dict) and (fname_b in dup_rec_dict):
                ...
                if (VERBOSE_OUTPUT == True):
                  pass
                  # print '    Swapped fields "%s" and "%s": "%s" <-> "%s"' %                      (fname_a, fname_b, fvalue_a, fvalue_b)
      ...
      while (num_modif_in_record < max_num_record_modifi):
        ...
        for m in range(num_field_mod_to_do):
          ...
          if (type_modification_to_apply == 'typ'):
            ...
            #
            if ((mod_op == 'misspell_prob') and                ('misspell_dict' in field_dict) and                (old_field_val in field_dict['misspell_dict'])):
              ...
              if (VERBOSE_OUTPUT == True):
                pass
                # print '    Exchanged value "%s" in field "%s" with "%s"' %                      (old_field_val, field_name, dup_field_val) +                      ' from misspellings dictionary'
            ...
            #
              ...
              if (field_dict['type'] == 'freq'):  # Frequency file based field
                ...
                min_digit = int('1' *                                (int(1+round(field_dict['num_digits']/2.))))
                rand_num = random.randint(min_digit, max_digit)
                dup_field_val = area_code+' '+                                str(rand_num).zfill(field_dict['num_digits'])
              ...
              if (dup_field_val != old_field_val):
                if (VERBOSE_OUTPUT == True):
                  pass
                  # print '    Exchanged value in field "%s": "%s" -> "%s"' %                        (field_name, old_field_val, dup_field_val)
            ...
            #
              ...
              if (VERBOSE_OUTPUT == True):
                pass
                # print '    Set field "%s" to missing value: "%s" -> "%s"' %                        (field_name, old_field_val, dup_field_val)
            ...
            #
              ...
              if (dup_field_val != old_field_val):
                if (VERBOSE_OUTPUT == True):
                  pass
                  pass
                  # print '    Swapped words in field "%s": "%s" -> "%s"' %                      (field_name, old_field_val, dup_field_val)
            ...
            #
              ...
              if (VERBOSE_OUTPUT == True):
                pass
                # print '    Exchanged missing value ' +                      '"%s" in field "%s" with "%s"' %                       (missing_value, field_name, dup_field_val)
            ...
            #
              ...
              if (rand_sub_pos != None):  # If a valid position was returned
                ...
                new_field_val = dup_field_val[:rand_sub_pos] + new_char +                              dup_field_val[rand_sub_pos+1:]
                ...
                if (new_field_val != dup_field_val):
                  ...
                  if (VERBOSE_OUTPUT == True):
                    pass
                    # print '    Substituted character ' +                          '"%s" with "%s" in field ' %                          (old_char, new_char) + '"%s": "%s" -> "%s"' %                          (field_name, old_field_val, dup_field_val)
            ...
            #
              ...
              if (rand_ins_pos != None):  # If a valid position was returned
                dup_field_val = dup_field_val[:rand_ins_pos] + rand_char +                              dup_field_val[rand_ins_pos:]

                if (VERBOSE_OUTPUT == True):
                  pass
                  # print '    Inserted char ' +                        '"%s" into field "%s": "%s" -> "%s"' %                        (rand_char, field_name, old_field_val, dup_field_val)
            ...
            #
              ...
              dup_field_val = dup_field_val[:rand_del_pos] +                              dup_field_val[rand_del_pos+1:]

              if (VERBOSE_OUTPUT == True):
                pass
                # print '    Deleted character ' +                      '"%s" in field "%s": "%s" -> "%s"' %                      (del_char, field_name, old_field_val, dup_field_val)
            ...
            #
              ...
              new_field_val = dup_field_val[:rand_trans_pos] + trans_chars2 +                            dup_field_val[rand_trans_pos+2:]
              ...
              if (new_field_val != dup_field_val):
                ...
                if (VERBOSE_OUTPUT == True):
                  pass
                  # print '    Transposed characters "%s" in field "%s": "%s"'                       % (trans_chars, field_name, old_field_val) +                        '-> "%s"' % (dup_field_val)
            ...
            #
              ...
              while (dup_field_val[rand_ins_pos-1] == ' ') or                  (dup_field_val[rand_ins_pos] == ' '):
                 rand_ins_pos = error_position(dup_field_val, 0)

              new_field_val = dup_field_val[:rand_ins_pos] + ' ' +                            dup_field_val[rand_ins_pos:]
              ...
              if (new_field_val != dup_field_val):
                ...
                if (VERBOSE_OUTPUT == True):
                  pass
                  # print '    Inserted space " " into field ' +                        '"%s": "%s" -> "%s"' %                        (field_name, old_field_val, dup_field_val)
            ...
            #
              ...
              new_field_val = dup_field_val[:space_ind] +                              dup_field_val[space_ind+1:]
              ...
              if (new_field_val != dup_field_val):
                ...
                if (VERBOSE_OUTPUT == True):
                  pass
                  # print '    Deleted space " " from field ' +                        '"%s": "%s" -> "%s"' %                        (field_name, old_field_val, dup_field_val)

          # -------------------------------------------------------------------
          # Phonetic modifications
          #
          elif ((type_modification_to_apply == 'pho') and                ('pho_prob' in field_dict) and (old_field_val != None)):
            ...
            if (random.random() <= field_dict['pho_prob']):
              ...
              if (',' in phonetic_changes):
                ...
                if (VERBOSE_OUTPUT == True):
                  pass
                  # print '    Phonetic modification ' +                        '"%s" in field "%s": "%s" -> "%s"' %                        (ch, field_name, old_field_val, dup_field_val)
          ...
          #
            ...
            # characters)
            #
            elif ((random.random() <= field_dict['ocr_fail_prob']) and                  (len(old_field_val) > 1)):
              ...
              if (VERBOSE_OUTPUT == True):
                pass
                # print '    OCR Failure character ' +                      '"%s" in field "%s": "%s" -> "%s"' %                      (del_char, field_name, old_field_val, dup_field_val)

            # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            # Random insertion of a space (thus splitting a word) (field must
            # contain at least two characters)
            #
            elif ((random.random() <= field_dict['ocr_ins_sp_prob']) and                  (len(dup_field_val.strip()) > 1)):
              ...
              if (new_field_val != dup_field_val):
                ...
                if (VERBOSE_OUTPUT == True):
                  pass
                # print '    OCR Inserted space " " into field ' +                        '"%s": "%s" -> "%s"' %                        (field_name, old_field_val, dup_field_val)

            # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            # Random deletion of a space (thus merging two words) (field must
            # contain a space character)
            #
            elif ((random.random() <= field_dict['ocr_del_sp_prob']) and                  (' ' in dup_field_val)):
              ...
              if (new_field_val != dup_field_val):
                ...
                if (VERBOSE_OUTPUT == True):
                  pass
                  # print '    OCR Deleted space " " from field ' +                       '"%s": "%s" -> "%s"' %                        (field_name, old_field_val, dup_field_val)
          ...
          elif ((old_field_val != org_field_val) and                (dup_field_val != old_field_val)):  # Following field mods.
            field_mod_count_dict[field_name] += 1
            num_modif_in_record += 1
